...
- Styling: Tailwind 3.4, institutional-minimal system per SYSTEM_DESIGN DR-12 — light-first paper base, first-class dark via tokens, ONE purple accent (`accent`, #5B5BD6), emerald/rose/amber strictly semantic (up/compliant · down · non-compliant), hairline borders + contact/ambient shadows, no glow/neon/backdrop-blur. Tokens live in `globals.css` + `tailwind.config.ts` — never per-component hex. Class merging: `clsx` + `tailwind-merge` are installed. RTL: logical properties ONLY in shared components (`ms-`/`me-`/`ps-`/`pe-`, `text-start`/`text-end`).
