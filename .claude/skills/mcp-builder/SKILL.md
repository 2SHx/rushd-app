...
An MCP server exposes tools to any MCP client (Claude Code, other agents). In this repo, MCP servers are single-file TypeScript stdio servers under `mcp/<name>/index.ts`, run directly with `tsx` — no build step, no nested package.json (deps live in root devDependencies).
...
4. Register in `.mcp.json` at repo root:
   ...
   ```
5. Restart the Claude Code session; verify with `claude mcp list` (must show connected), then call one tool live.
