...
Defects fixed (all three were being read as environmental alongside genuine Neon timeouts): `evaluateEpisodeOutcome` checked acceptance before safety, filing a simultaneous safety violation under the milder ACCEPTANCE_FAIL code; `renderProposedContext` silently TRUNCATED an oversized lesson at a byte boundary while still presenting it as the proposer's exact verified text ("never do X unless Y" -> "never do X"); and it rendered `status: UNADMITTED` for versions already carrying a later ADMIT or REVOKE.
Verify: suite 11 failed -> 7 -> guard test de-flaked (5497ms -> 6ms, no DB); continual-harness 48/48 and continual-benchmark 77/77; lint and tsc clean. Isolated the evaluator self-seal by reverting it alone: 1 failure at HEAD vs 21 uncommitted — `verifyScorecard` requires the git blob at `scorecard.evaluatorCommit` to be byte-identical to the running file, so an evaluator change cannot be verified until committed, and the review fixture hash must be refreshed with it (4th such refresh).
Open: `portfolio.test.ts`/`quant-eval` still fail on latency alone — the fix is a disposable Neon branch with RUSHD_DISPOSABLE_TEST_DB=1, which the guard already recognises. `computePortfolioMetrics.irVsSpy/irVsSpus` remain low by exactly sqrt(252), pinned at metrics.test.ts:255-268; published blast radius is zero (no terminal card carries an IR key) and the cost of fixing is one golden string in qdr19Additivity.test.ts:42.
...
Falsified (both real mechanism results, neither inert-by-accident): exit-only catastrophic stop — fired 11 times in 8.5 years, tail 68.40%→67.13%, `stopLossPct` measurably inert; annual-fundamentals selection gate — changed 86.84% of decision weeks (median 5 skips) yet OOS DSR fell 0.874→0.680 while CAGR rose to 127.99%, because `ADMIT_UNCONDITIONED` inverted into a bet on filing ABSENCE (excluded SMCI at momentum rank 1 on a ~300-day-stale 10-K, promoted XOM at rank 9 with zero filings).
Measured and closed: intraday is dead (6 strategies / ~2,040 trades, all negative); overnight-only is a real effect (14.36%/yr, Sharpe 1.139) that still loses to buy-and-hold (23.67%, Sharpe 1.170, zero trades), break-even 2.892 bps/side; perfect weekly top-5 selection ≈500%/yr vs the engine's 36.99% full-period, so the binding constraint is SIGNAL QUALITY, not opportunity; DSR values across the ledger were computed at varying N and are NOT comparable (fast-momentum's 0.874 was N=9; at N=108 it is ~0.544).
Open: INCIDENT — a concurrent agent's `prisma migrate dev` rolled back 6 migrations and truncated the shared DB (`MarketBar` 263,815/229 → 8,760/4, `Fundamentals` → 0); fundamentals fully rebuilt, bars restoring; nothing irreplaceable lost ONLY because `results/` was un-gitignored 2 days earlier so the recovered fast-momentum NAV was committed. RULE: dispatches are fenced by file, never by resource — run ONE DB writer at a time and ban `migrate dev` in any dispatch not about migrations. Production cron never calls `/api/cron/quant-run`, so AUTO_PAPER cannot run unattended. The SEALED lane must never be run or inspected before its window closes.
...
## 2026-08-07 — QDR-11 DIVERSIFICATION class + 3 self-inflicted defects fixed ✅
Shipped: QDR-11 — a COMPARATIVE diversification criterion set for universe-construction claims: **(D1)** effective-bet ratio ≥1.10 mean and ≥1.00 every cycle on realized FORWARD returns, **(D2)** book-volatility reduction ≥5% plus a paired seeded moving-block-bootstrap 95% upper bound ≤0.95 — both against the **incumbent rule re-run at the same formation dates, same universe, same sleeve size**. Terminates `ACCEPTED_DIVERSIFICATION` with `acceptanceMeaning: 'UNIVERSE_RULE_ADMISSION_ONLY'`: **no capital, no allocation, no tournament entry** — only permission for a FUTURE version to adopt the rule, which must then earn its own ALPHA/BETA verdict. Roadmap row G9 added.
Decisions: Neither existing class could host it — ALPHA refused at seal on three independent axes (OOS DSR 0.93<0.95, MC p95 41%>30%, n=90<100) and BETA criterion (a) is an ABSOLUTE band a governor-less book cannot honestly declare (its half-window test fails on regime alone: ~40% realized vol in 2020 vs ~14% in 2021, same rule). **NO RETURN CLAIM EVER** — the return improvement carries IR 0.111–0.263 ⇒ `years = 6.1826/IR²` ⇒ **89–503 years**, permanently unprovable, so it is forfeited by design and the ×1.13 appears as motivation only. Anti-gate-shopping rests on there being **nothing to shop for**: zero return claim, zero capital, plus `productClass` inside `stableConfigHash` and a named prior sealed `comparatorVersionId` with exactly one changed variable. The candidate's own predicted 41% MC p95 means it still terminates REJECTED — passing D1+D2 while failing the breaker never yields an ACCEPT.
Verify: tsc clean; `src/quant/universe` + `src/quant/backtest` 335 passed / 1 pre-existing Neon quant-eval timeout.
Open: THREE DEFECTS IN MY OWN COMMITTED CODE, caught at director review, fixed in `8907f3e` — (1) `runLab` resolves the sleeve ONCE at `to` and replays backwards, which for correlation selection is **direct look-ahead on the exact statistic being claimed**; rolling annual PIT re-formation on trailing 252 trading days is now mandatory for BOTH arms or the run earns `DATA_QUALITY_PIT_FAILURE`; (2) the wrapper passed 504 CALENDAR days and used every return (~347 trading days), silently a different mechanism from the validated 252-day protocol — now truncated; (3) no breadth-floor guard, so a binding sector cap could return 9 names against a declared 15 floor. **COMPARATOR-INTEGRITY CONFESSION recorded in QDR-11**: my first measurement's 29.6% volatility reduction was a 2.6× overstatement caused by comparing against the most-correlated-20 instead of the real dollar-volume sleeve; the honest figure is **11.6%** (still detectable at n≈407 daily observations against 644 reserved).

## 2026-08-07 — Sleeve selection on correlation + sector balance, OOS-validated ✅
Shipped: `src/quant/universe/decorrelatedSleeve.ts` — greedy minimum-average-correlation selection under a per-sector cap, with liquidity demoted from OBJECTIVE to GATE (caller pre-filters to a tradeable pool; you can never select a name you cannot trade). Pure, deterministic, symbol-ascending tie-breaks, 10 tests.
Decisions: The measured problem — 98 names at average pairwise correlation **0.326 = ~3.0 effective bets**; ranking by dollar volume selects the mega-cap growth names that move together, so the universe rule was actively producing the concentration every strategy then failed on. Greedy not exhaustive on purpose: optimal k-subset is combinatorial and squeezing the last decimal would be fitting the correlation matrix. A binding sector cap returns a SHORT sleeve rather than breaching the cap to hit a count. Also fixed a real robustness bug found by test: `pearson` on a constant series returned 7.08e-17 from rounding error instead of 0 (`[0.05,0.05,0.05]` leaves sxx~1e-34) — now uses a relative variance floor.
Verify: **WALK-FORWARD OOS** — select on trailing 252d, measure on the next 252d never seen, 8 formation dates 2017→2024: the selected-20 sleeve had LOWER correlation in **8 of 8 periods**; mean effective-bet ratio **1.28×**, implied Sharpe multiplier **1.13×** against the 1.18× in-sample hindsight claimed. tsc + lint clean; `src/quant` 1,078 passed / 5 pre-existing DB-bound failures (4 portfolio + quant-eval, none referencing the changed modules).
Wired: `selectCorrelationBalancedSleeve` in `sleeveSelector.ts` (liquidity pool via the existing dollar-volume path, then decorrelate inside it) + an OPT-IN `CORRELATION_BALANCED_SLEEVE_SETUP_IDS` in `runLab.ts` that is deliberately EMPTY — every existing id has a published card produced under dollar-volume selection, so switching a sealed setup's universe is a research decision recorded in its preregistration, never an implementation default. `SYMBOL_SECTOR` exported for the sector map. Runs print ρ and effective bets. Honest ceiling unchanged: NASDAQ-only halal equities share one dominant factor, so ~0.75 Sharpe / low-to-mid-teens CAGR is the realistic top, and dropping TASI costs about a third of the available diversification benefit (×1.26 → ×1.13).

## 2026-08-07 — ORCL guard: backdated Sharia evidence can no longer certify a run ✅
Shipped: `backdatedShariaEvidence(entries, periodStart)` + a `periodStart` argument to `buildC1ShariaRunSnapshot`, wired in `runLab.ts` with the run's `from` date. A name whose evidence `asOf` post-dates the first decision now resolves `compliant: null` (UNKNOWN, not haram), which `deriveShariaState` already maps to `UNSCREENED_EXECUTION_BLOCKED` — so the report card fires `SHARIA_UNVERIFIABLE` instead of asserting compliance it cannot evidence. 5 regression tests pin the exact ORCL scenario.
Decisions: The user found this independently — ORCL sits in the 2026-07-17 SPUS holdings at 0.56% and that single dated snapshot was certifying every trade back to 2018, because the builder hardcoded `compliant: true` and took `asOf` from the run's END date. Wrong even when profitable: a Sharia-integrity defect, not a statistical one. `null` not `false` — the name is not proven non-compliant, its compliance is UNKNOWN for that period. ANY single backdated name blocks the whole run; no partial certification. Evidence dated exactly on the first decision counts as valid.
Verify: tsc, lint clean; `src/quant/backtest` + `src/quant/strategies` 637 passed / 1 pre-existing Neon quant-eval timeout; sharia suite 12/12.
Open: **CONSEQUENCE — every prior terminal card's `C1 VERIFIED_COMPLIANT` claim was overstated** and would now read `UNSCREENED_EXECUTION_BLOCKED`; the cards stand as historical record but the compliance line must not be re-quoted as evidence. Sharia authority stack fixed by owner decision: AAOIFI + Al-Rajhi Sharia Committee + S&P Shariah (Tier-1 label only); **دار الإفتاء المصرية is excluded** as too permissive for a Gulf product. Al-Rajhi's published thresholds (5% interest income ÷ revenue, 30% debt ÷ market cap) MATCH `tier2AaoifiScreener` exactly — the repo is stricter still, so the thresholds were never the defect, only the dating. التطهير = non-compliant income ÷ total revenue applied to dividends; HLAL's own published figure is exactly 5% of dividends.

## 2026-08-07 — EXPLORATORY: the volatility governor adds ~0 return over holding less ⚠️
Shipped: No code. An exploratory measurement on REAL 10-year daily bars (98 names with a full 2016-07-18→2026-07-17 spine, out of 229 symbols in Neon) answering whether the vol-targeting "enhancement" is worth committing a forward window to. **Survivor-conditioned (2026 SPUS holdings applied backwards), so the CAGR LEVEL is not trustworthy — the RATIOS are.**
Decisions: Equal-weight buy-and-hold baseline CAGR 23.02% / vol 20.2% / maxDD 32.1% / **Sharpe 1.14**. Vol-targeted at {8,10,12,15}% all land at **Sharpe 1.17** — vol targeting is a RISK DIAL, not an alpha source, exactly as the director predicted (Sharpe-neutral by construction). Against the honest control of simply holding LESS: static 50% sleeve gives Sharpe 1.14, and at matched volatility the dynamic governor's raw edge is **+0.30pp CAGR / −2.6pp drawdown** — but its own turnover is **1.86×/yr = 0.28pp** cost drag, so the **NET return advantage is +0.02pp per year**. The drawdown reduction is real and survives; the return advantage does not.
Verify: real Neon bars, no fixtures, no synthetic series; B1-correct estimator (vol measured on the UNMANAGED sleeve, applied next day, no leverage, cash at 0).
Open: This does NOT invalidate the BETA lane — QDR-10 says a BETA product claims risk control and NOT edge, and that is precisely what was measured, so the finding CONFIRMS the framing while killing any hope of "great results". Every enhancement family is now tested and none beats a diversified basket by much: momentum (falsified, worse tails), risk parity, sector caps, Markowitz, vol targeting (~0 net). The honest product is a diversified halal basket sized to a risk tolerance; the complexity is not adding return. Baseline Sharpe 1.14 is survivor-inflated — the director's honest ex-ante is 0.65.
...
Verify: A/B at identical seeds — buggy drift ratio 0.886/87.1% power/8.8% neg-blocks vs corrected 0.993/55.5%/46.6%; director's independent reconciliation in one harness agrees (0.836→0.985 on drift alone; exposure timing, beta and tracking error each move it <0.01); tsc clean.
Open: G8 follow-up DONE at `979fb06`. First honest BETA verdict now ≈2 years (n=104), a ~7× gain on 14.72y; the earlier 3–4y figure was wrong and the 1–2y figure was right for the wrong reason.
NEXT SESSION, in order: (1) DONE — `runLab.ts` now resolves `productClass` and assembles `betaEvidence` from the run's own series via the new `betaEvidence.ts` (session-keyed benchmark alignment on the frozen five-session block convention, blocks dropped from BOTH series when either boundary close is missing, no forward fill; OLS beta; turnover/cost drag derived from the engine's own charged per-side cost, now exported). Fails closed when the gate block is half-declared. (2) DONE / **CORRECTION** — Neon is NOT unreachable; that earlier claim was wrong. It connects in 2.7s and holds **1,200 SPUS daily bars (2021-10-05 → 2026-07-17)**; `quant-eval.test.ts` is simply a slow test hitting a 15s hook timeout, not a connectivity failure. Running the QDR-10 falsifier on those real bars gives **40.2% negative blocks over 239 five-session blocks** (realized vol 18.96%, CAGR 15.03%, Sharpe 0.79) — OUTSIDE the shipped 42–45% band, which was itself the bug: one SE at n=239 is **3.20pp** and the band is only 3pp WIDE, so it would have refused the very index the lane benchmarks against. Fixed by `negativeBlockFractionBand(n)` widening the long-run band by 3 SE at the lane's own observation count; real 40.2% now passes (0.87 SE from 43%) while the drift bug's 8.8% stays refused at 10.7 SE. Consequence recorded in test: at n=104 a declared 0.50 now SEALS, because one SE there is 4.9pp and 0.50 is statistically indistinguishable from the reference. (3) director preregisters the SPUS lane as BETA — it is the leading candidate because it is confirmed twice to bypass the engine governor (`targetWeight` setups take a TARGET order that overwrites the trim order and carry no `targetGrossFraction`), so B1 never reached it and it has zero tracking error. (4) i18n-fintech-expert pass for the Arabic capture annotation + amended disclaimer into `messages/*.json`.

## 2026-08-06 — G8 BETA gate implemented + two defects caught pre-seal ✅
Shipped: `betaCriteria.ts` (vol band via one-sided 95% log-variance bound, convexity + seeded moving-block capture-gap bootstrap, beta-scaled shortfall, cost/turnover envelope), `productClass` resolution (absent ⇒ ALPHA), `ACCEPTED_BETA`/`REJECTED_BETA` with protocol-level class↔label enforcement, and BETA card copy incl. the no-edge disclaimer — `ACCEPTED_BETA` is now reachable in code.
Decisions: BETA gate blocks do NOT require `hypothesizedAnnualSharpe` (demanding an edge estimate from a class forbidden to claim one is incoherent; QDR-10's "alongside QDR-9's keys" reads as per-class — architect to align wording); a BETA card with no criteria evidence fails closed on all four; DSR still printed but annotated "reported, not a gate".
Verify: tsc, lint clean; `src/quant/backtest` 235 passed / 1 pre-existing Neon quant-eval timeout; regression sweep over strategies+portfolio clean apart from the pre-existing prisma `portfolio.test.ts`.
Open: TWO REAL DEFECTS found by the director BEFORE any seal — (B1) `portfolioEngine.ts:965-969` derives `realizedVolAnnual` from the book's OWN exposure-scaled NAV, a closed loop whose fixed point delivers √(T·σ) not T (10% target ⇒ ~14.9%); affects `halalManagedMomentumCore` + three `tsMomentumHalalBasket` versions (all terminal, no published result changes), NOT the SPUS lane which computes vol strategy-side; needs a fix + test before any new vol-targeted lane. (B2) criterion (c1) is structurally undetectable for a 40-name sleeve measured against a broad fund — power ≤26% at any window — so the risk-parity BETA lane was REFUSED. QDR-10's calibration (ratio 0.88–0.92) and the director's regime model (0.951 at zero tracking error) CONTRADICT each other; reconciliation is in flight and (c1) may need re-deriving by the architect. `runLab.ts` does not yet pass `productClass`/`betaEvidence`, so every run today is still ALPHA.

## 2026-08-06 — QDR-10: product class ALPHA|BETA, matched promotion criteria ✅
Shipped: QDR-10 — `productClass` declared irreversibly at seal (inside `stableConfigHash`, so a post-seal change breaks QDR-9 condition (a); absent ⇒ ALPHA fail-closed); BETA replaces ONLY the DSR-vs-zero gate with volatility-control / drawdown / capture-convexity / cost criteria, terminates `ACCEPTED_BETA`, and may make no alpha claim anywhere.
Decisions: QDR-6's DSR gate asks "is there an edge?" — the wrong question for a book claiming risk-managed exposure, and answering it needed 14.72y at the director's honest ex-ante Sharpe 0.65 (it refused the measured 1.13 as survivor- and regime-inflated); thresholds are MEASURED not derived (4,000-path GARCH sim, no look-ahead): convexity `downCapture/upCapture < 0.95` because the true ratio converges to 0.88–0.92 and a 0.90 threshold would fail a working mechanism ~half the time; BETA's power gate is STRICTER than ALPHA's (hard ≥85% on the binding criterion, simulation-established, closed form refused as measured-optimistic).
Verify: tsc clean after both records; convergence measured at n=52 (1.03y) 5.1% vol false-alarm / n=156 (3.10y) 87.1% convexity / n=208 (4.13y) 92.1%, against 742 obs for the Sharpe claim — a ~14× cheaper test, first honest verdict 3–4 years not the 1–2 I first estimated (superseded in-text).
Open: `computeCaptureRatios` is module-local at `metrics.ts:241` and must be exported (G8); the bilingual no-edge banner needs an i18n-fintech-expert pass before it reaches `messages/*.json`; cross-market data breadth remains the ONLY route to a genuine ALPHA ACCEPTED and QDR-10 must never be cited as a reason to stop pursuing it.
...
## 2026-08-07..09 — QDR-11..13: the DIVERSIFICATION lane, and what review found in it
Shipped: G9-PIT (rolling point-in-time sleeve re-formation, `pitSleeveSchedule.ts`, opt-in `policy.sleeveMembership`) · G9 (`diversificationCriteria.ts` D1/D2/plateau, `ACCEPTED_DIVERSIFICATION`, `UNIVERSE_RULE_ADMISSION_ONLY`) · G9-EVIDENCE (`diversificationEvidence.ts`, the two-arm measurement) · QDR-12 + closed historical-anchor registry · QDR-13 + G10 (sealed gate block reachable at run time) · `halal-decorrelated-risk-parity-core@v1` setup + preregistration DRAFT · `pitShariaVerdict.ts` + 3,813 real SEC filings backfilled.
Decisions: the comparator is the incumbent RULE re-run at the SAME formation dates, never its sealed card · `productClass` stays hash-anchored; absent still resolves ALPHA · anchors are IDENTITY-only, never evidence, and the set is closed by a date that already happened · annual-only (10-K/FY) Sharia granularity — a verdict changes at most once a year against a monthly rebalance, a modelling choice not a technical limit.
REVIEW FOUND 18 DEFECTS, MOST IN WORK THE MAIN SESSION HAD WRITTEN AND WAS CONFIDENT ABOUT. The load-bearing ones: the treatment arm never passed its own sectorCap/poolSize and silently ran selector defaults (poolSize 120 vs the declared 80 — the book measured was not the book sealed); the sealed gate block never reached runLab at all, so BETA and DIVERSIFICATION both resolved ALPHA; `plateauCells` cardinality was unenforced so a 1-cell grid passed as a 9-cell plateau; QDR-12 claimed closure "by arithmetic" when it was really code review; and 50.2% of the first fundamentals backfill were quarter-fragments while the PIT reader had an undefined tiebreak across 79% of rows.
Lesson worth carrying: checking the property you are worried about is not the same as checking the data is good — 0/6,917 look-ahead rows was true, and half those rows were still unusable. Every one of these was found by dispatching a fresh reader, never by re-reading own work.
Open/BLOCKERS: (1) LIFECYCLE/DELISTING DATA DOES NOT EXIST — `spusNport.ts` hard-codes `lifecycleCoverage:{delisted:false,suspended:false}`, so the terminal PIT fence cannot lift and every terminal run still aborts; options are a keyless SEC Form 25 backfill (recommended), a paid vendor, or declaring the survivor bias in a design record. (2) Membership history is 3 N-PORT snapshots (2020-2021), needs the quarterly backfill. (3) Arabic card copy for QDR-11 owed, i18n-fintech-expert gated. (4) XOM unscreenable — SEC maps it to a new CIK with no annual 10-K history. (5) `halal-causal-tcn-alpha` and `halal-spus-vol-managed-beta` are stuck at CODIFIED: rich configs, no `runConfig`, and re-sealing would fail QDR-9(d) now their evidence boundaries have opened.
Standing caveat: none of this moves the return ceiling. `halal-decorrelated-risk-parity-core@v1` is PREDICTED to terminate REJECTED_DIVERSIFICATION on the 30% drawdown breaker by design — it adjudicates a structural claim and may never make a return claim. The measured ceiling for NASDAQ-only halal equities remains ~0.75 Sharpe / low-to-mid-teens CAGR.

## 2026-08-16 — G11 continuation: honest N-PORT identity/lifecycle wiring (DRAFT-safe, not seal-ready)
Shipped: classified the 37 OpenFIGI name mismatches (24 current-roster holdings, 13 held outside the roster, NOT_HELD=0); added a 24-entry hash-pinned curated map; expanded SPUS coverage to 25 gap-free quarterly N-PORT snapshots; added content-bound Form-25 lifecycle injection, UNKNOWN-not-OUT handling, bilingual QDR-10/11 copy, deterministic portfolio fixtures, and removed the completed one-time PIT fragment repair script.
Decisions: unresolved held identities remain `MEMBERSHIP_UNKNOWN`; delisting coverage is evidence-bound and unwaivable; suspension remains uncovered and only the declared DIVERSIFICATION waiver can pass it; the candidate stays `DRAFT`/`configHash:null`, with `from=2020-05-31`, OOS 0.50, and `symbols:null` until the exact lifecycle-complete roster exists. A new seal guard refuses DIVERSIFICATION manifests without explicit symbols, but deliberately does not pretend a non-empty list proves the missing 161-name evidence.
Verify: focused G11/i18n/backfill suites 73/73, N-PORT/OpenFIGI corrections 29/29, seal wiring 21/21, portfolio 8/8; tsc, lint, and diff-check clean; C1 QA PASS, C2 safe-to-commit-as-DRAFT PASS / ready-to-seal FAIL, C3 QDR-14 ratified with explicit open condition. A direct full-suite run reproduced three unrelated failures: authz tier-matrix assertions ×2 and Academy cross-user progress authorization; the earlier quant-eval timeout did not recur.
Open/BLOCKER: capture and byte-pin the exact 161-symbol Form-25 coverage artifact, inject it through the real terminal run path, freeze that list in the manifest, compose independent PIT Sharia evidence, and render survivorship disclosure plus eligible/excluded/crosswalk counts before any seal or FULL claim. Preserve DB-reconciliation evidence if the deleted PIT correction utility is ever needed again. The 30% drawdown breaker still predicts `REJECTED_DIVERSIFICATION`.
