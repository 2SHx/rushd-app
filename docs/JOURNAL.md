...
Open: Not yet wired into `runLab`/`sleeveSelector` — this is the pure core plus its validation; the caller-side swap from `selectDollarVolumeSleeve` is the next step and needs a sector map (`SYMBOL_SECTOR` is module-private in `halalSectorCappedRiskParityCore.ts`). Honest ceiling unchanged: NASDAQ-only halal equities share one dominant factor, so ~0.75 Sharpe / low-to-mid-teens CAGR is the realistic top, and dropping TASI costs about a third of the available diversification benefit (×1.26 → ×1.13).
