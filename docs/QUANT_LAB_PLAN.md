...
| G3e shared-cash daily strategy book | quant-strategist | ⏳ R3-0 NEXT | DB-free monolith-internal `portfolioEngine.ts`: date-major shared cash/NAV/real positions, canonical-symbol entry priority frozen before OOS, real portfolio state through `applyEnvelope`, true daily NAV/drawdown; legacy independent route unchanged |
...
1. **G3e portfolio truth prerequisite:** normalized weekly model/SPY/SPUS evidence already lives in the
   existing `BacktestRun.metrics` envelope. Build the DB-free shared-cash daily strategy-book engine first,
   then persist its true portfolio NAV/drawdown plus strategy version/effective parameters; this unlocks
   basket-governor validation, investable-style NAV, drawdown and Monte Carlo fan charts without invented data.
...
| # | Unit | Spec (binding) |
|---|---|---|
| R3-0 / G3e | **shared-cash daily strategy book** (prerequisite) | Add only `src/quant/backtest/portfolioEngine.ts` + focused tests and explicit `scripts/backtest.ts` routing. DB-free, deterministic and date-major; one shared cash/NAV/positions state; matched same-day entries consume cash in canonical-symbol ascending order frozen before OOS; true daily NAV/drawdown output; existing `applyEnvelope` receives real positions. Exit gate: shared-cash conservation after every fill; real positions reach the envelope; stable tie-break replay; configured cap proves ≤6 simultaneous holdings; PIT future-bar injection fails; daily NAV replay is identical; MOCK inputs fail; legacy independent setup artifacts remain byte-identical unless explicitly routed. Fixed check: `npx vitest run src/quant/backtest/portfolioEngine.test.ts --no-file-parallelism`. No dependency/schema/service/queue/API. |
| R3-1 | **momentum-v3** (strongest candidate's shot; depends on R3-0 PASS) | Import ts-momentum-halal-basket-v2's entry/exit family unchanged and add ONE new a-priori hypothesis layer: basket-level exposure governor — total deployed fraction scaled so trailing 60d realized BASKET vol ≤15% annualized (down-only to cash, never >1×) AND max 6 concurrent positions. The governor is a book policy atop `portfolioEngine.ts`, never a single-symbol `sizeFraction` claim. Target: MC maxDD p95 ≤30% + OOS DSR ≥0.95. Files: new `src/quant/strategies/tsMomentumHalalBasketV3.ts` + tests, catalog, ledger; `src/quant/backtest/portfolioEngine.ts` seam and `scripts/backtest.ts` shared-route wiring only; v2 and the legacy route stay untouched. Acceptance additionally proves realized 60d basket vol scaling and ≤6 daily holdings on the true shared-book NAV. Fixed run: `npm run backtest -- --setup ts-momentum-halal-basket-v3 --from 2018-01-02 --to 2026-07-10 --seed 42`. |
