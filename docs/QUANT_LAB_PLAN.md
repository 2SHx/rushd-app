...
| R3-2 | **dual-momentum-rotation — ✅ DONE / REJECTED** | Final continuous-cap shared-book run (`d48638c`, BacktestRun `d3628d85-43a8-43c2-a327-44f642104729`, seed 42): exact seven assets / 14,108 real bars / 0 MOCK / 2,141 NAV days; 147 closed execution records (partial risk trims included); full/OOS CAGR 5.49%/13.85%, DSR(9) 0.737/0.701; MC book-day p95 DD 23.15%, max post-fill gross 25%, max one holding. Frozen 3×3 plateau FAIL (294d neighbors degraded 50.66% > 50% bound). Ordered reasons: `DSR_FAILURE`, `NO_PROFIT_PLATEAU_OVERFIT`, `SHARIA_UNVERIFIABLE`; no AUTO_PAPER. Invalid data-deletion run `abe70156…` and entry-only-cap diagnostic `7fc795e8…` are superseded, not evidence; next R3-3. |
