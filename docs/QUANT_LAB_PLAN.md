...
1. **Evidence persistence:** normalized weekly model/SPY/SPUS evidence now lives in the existing
   `BacktestRun.metrics` envelope and was backfilled only through reproducible reruns. Next persist a true
   shared-cash daily portfolio/drawdown series plus strategy version/effective parameters; this unlocks
   investable-style NAV, drawdown and Monte Carlo fan charts without invented data.
