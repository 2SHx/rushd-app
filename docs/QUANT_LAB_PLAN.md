...
| G1 intraday data spine | data-engineer | 🔄 IN FLIGHT | additive `IntradayBar`+`SymbolSnapshot` (Decimal OHLCV, @@unique per QDR-6), Alpaca minute-bar resumable backfill `scripts/backfill-intraday.ts`, REAL-bar fixture capture `scripts/capture-intraday-fixtures.ts` (6–10 verified symbol-days ≤2MB committed), Yahoo keyless tier labeled, signed cron `/api/cron/quant-intraday`, 12h-universe-cache bypass, PIT tests |
