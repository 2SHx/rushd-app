...
capped INCUBATION books labeled **"unpromoted forward test — paper only"**; (2) the Alpaca paper
account is user-reset to **$1,000,000** (clears the −$82.8k margin debt; envelope still limits by
CASH, never buying power); (3) the verified halal universe is built this phase at **zero data cost**
...
| E4 | **bagholder-bounce v1** | quant-strategist | Pre-registered ledger CANDIDATE (gap-down ≥20% + flush + first higher low, long-only), executed **behind the daily modes** (only after E1–E3 verdicts). Intraday QDR-6 lane rules bind: captured-real data only, participation-cap fills, envelope-clamped. Pre-register the hypothesis against the seven prior intraday long-only falsifications (STRATEGY_LAB cards) — state a-priori why this differs. `--diagnostic`/1Y first; ONE FULL terminal run; QDR-7 card + ledger + JOURNAL + commit; seed 42; no tuning against viewed OOS. |
| E5 | **time-of-day v1** | quant-strategist | Pre-registered LAST (after E4): 9:45-reversal + first-hour trend-lock codified as entry-window **constraints/A-B overlay** per its ledger row, long-only, intraday QDR-6 lane rules. Behind the daily modes. Pre-register vs the parents'/lane's ordered rejection history; `--diagnostic`/1Y first; ONE FULL terminal run; QDR-7 card + ledger + JOURNAL + commit; seed 42; no tuning against viewed OOS. |
