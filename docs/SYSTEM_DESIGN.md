...
### DR-20: Evidence-gated continual agent harness — immutable kernel, reviewed append-only lessons, bounded scoped retrieval
Decision: implement the project-wide continual harness as repo-local Node-built-in development tooling that preserves an immutable safety/evaluator kernel, activates only independently evidence-reviewed append-only lesson events, injects a deterministic bounded scoped lesson projection through the shared dispatch/context renderer, and rolls lessons back by append-only revocation while Git remains the code rollback mechanism.
Options considered: do nothing and rely on Git plus `docs/JOURNAL.md` (durable history but no structured, scoped, evidence-gated refinement) / adopt Prime Agent or a hosted memory service (new dependency and service, raw-context risk, and scale RUSHD does not need) / add a repo-local hash-chained event ledger and renderer over existing dispatch/review seams (chosen — no runtime service, datastore, API key, or dependency).
Rationale: Prime Agent inspires the propose→evaluate→admit refinement loop but is neither a dependency nor a runtime component; all specialist roles already pass through `scripts/dispatch.mjs`, direct orchestrators already load `AGENTS.md` or `CLAUDE.md`, and the existing independent review chain already produces the evidence that one development-time ledger and renderer converts into reproducible refinements without changing the Next.js runtime architecture or replaying transcripts.
Consequences: the immutable kernel is the base role prompts, `AGENTS.md`/`CLAUDE.md` house rules, this system-design contract, the event schema/admission evaluator, and every Sharia, risk, money, authz, no-key, child-safety, and quant-evaluation rule; a lesson can advise execution but can never override or edit that kernel, and kernel changes use the ordinary reviewed Git workflow outside the harness. The sole lesson memory is `.agents/continual/events.jsonl`: every SHA-256-linked event carries `schemaVersion`, `eventId`, `action` (`PROPOSE`, `ADMIT`, or `REVOKE`), `lessonId`, `lessonVersion`, scoped `roles`/repo-relative `paths`/`taskTags`, lesson text or revocation reason, `proposedBy`, `reviewedBy`, evidence command + exit status + bounded captured output + output digest + artifact/commit, `prevHash`, and `eventHash`; committed lines are never edited or deleted. `.agents/continual/head.json` is a committed `{eventCount, headHash}` checkpoint: verification runs before every dispatch/context render, every CLI mutation advances the ledger and checkpoint together, and any count/hash mismatch fails closed. The checkpoint detects deletion or valid-prefix truncation unless an attacker maliciously rewrites both ledger and checkpoint; the hash chain cannot prove completeness against that joint rewrite, so reviewed Git diff/history is the authoritative detector. Evidence captured output is capped at 16,384 UTF-8 bytes and its SHA-256 must equal `outputDigest`; `artifact` is `commit:<40-hex-full-sha>` and must pass `git cat-file -e <sha>^{commit}` locally. The admission CLI either captures command output or accepts already reviewed output, but both paths persist and digest the same canonical bounded bytes; these checks prove internal evidence/artifact consistency, not reviewer identity or review quality. Any agent may append `PROPOSE`, but `ADMIT` requires recorded `reviewedBy` provenance distinct from `proposedBy`, a PASS for the same lesson version, and an approved Git commit; `reviewedBy` is attribution rather than cryptographic authentication, the reviewed commit is the admission authority, reviewers remain report-only, and agents are forbidden to self-admit. Machine checks reject known kernel-keyword conflicts as defense in depth, while independent review and Git approval are authoritative for semantic kernel conflicts that keyword matching cannot completely detect. The active projection is admitted versions minus later revocations; before scope matching, query paths must be repo-relative normalized forward-slash paths, and absolute paths, backslashes, or any `..` segment are rejected. The renderer injects nothing when the target query path is the same as, a path-segment ancestor of, or a path-segment descendant of an immutable-kernel path; all other retrieval matches only the current role plus declared path/task scopes, sorts by scope specificity then `lessonId` and numeric `lessonVersion`, injects at most 8 lessons and 4,000 UTF-8 characters, and injects nothing when no lesson matches. `scripts/dispatch.mjs` calls the one renderer for every specialist, while `AGENTS.md` and `CLAUDE.md` require direct orchestrators to call the same renderer once per task; each injected lesson is delimited below the immutable kernel, includes `lessonId@lessonVersion`, evidence artifact/commit and reviewer attribution, and contains distilled guidance only — raw transcripts and hidden chain-of-thought are never stored or replayed. Admission rejects opening or closing delimiter variants after lowercasing and removing all whitespace, and rendering JSON-string-escapes lesson text before concatenation so lesson content cannot create control lines. A `REVOKE` event removes a lesson on the next render without rewriting history; a faulty harness/code change is reverted with Git, and ledger verification rejects a broken hash chain or checkpoint.
Revisit when: the ledger exceeds 500 admitted lesson versions or deterministic retrieval exceeds 100 ms on CI hardware; then replace only the repo-local index format, preserving the immutable kernel, evidence admission, bounded projection, provenance, and append-only revocation invariants.

### DR-21: Frozen paired benchmarks gate continual-agent improvement claims
Decision: require every continual-harness improvement claim to pass a local, provenance-bearing baseline-versus-candidate benchmark on frozen cases before DR-20 admission, while keeping benchmark mechanics deterministic and live-model quality claims evidence-based.
Options considered: rely on reviewer judgment alone (not measurable across episodes) / unit-test the evaluator and call the lesson improved (tests mechanics, not stochastic model quality) / pair repeated real dispatches on identical hash-pinned cases and apply a safety veto plus a minimum score delta (chosen — auditable without Prime Agent, a service, database, or dependency).
Rationale: RUSHD already has Node built-ins, Git, Vitest, `scripts/dispatch.mjs`, and the DR-20 ledger; a small paired evaluator uses those seams, isolates model variance through repetition, and prevents a persuasive single transcript from becoming evidence of improvement.
Consequences: each canonical-JSON case contains `caseId`, positive `caseVersion`, `role`, `task`, `primaryPath`, `taskTags`, `startCommit`, ordered `acceptanceCommands`, a non-empty rubric with unique IDs, and ordered `safetyCommands`; `caseHash` is SHA-256 of the DR-20 canonical serialization excluding `caseHash`, and `suiteHash` is SHA-256 of sorted `caseId@caseVersion:caseHash` lines. The computed `suiteHash` must equal a literal pinned in `scripts/continual-benchmark.mjs`; manifest and case hashes are integrity inputs, never authority, and changing a case, manifest, or pin requires an ordinary reviewed Git change. Commands are parsed to argv and executed without a shell; the complete grammar is only `npx tsc --noEmit` or `npx vitest run <normalized repo-relative *.test.ts>`, with exactly one test path and no options, glob, shell token, or control character. M16 freezes exactly two tracer cases; a proposal targeting an uncovered role cannot claim improvement or be admitted until a hash-pinned case for that role is reviewed and added. Each episode uses an OS-temporary Git worktree at its pinned `startCommit`; changed files are limited to the case `primaryPath` plus test paths named by its acceptance commands, the non-empty in-scope patch is committed as `Rushd Benchmark <benchmark@rushd.local>`, and `refs/continual-benchmark/<runId>/<caseId>/<arm>/<index>` retains the commit until QA reviews it, after which only that worktree/ref is cleaned. The evaluator runs three real episodes per case per arm through the same frozen runner, model, effort, task, environment, and commands with dispatch fallback disabled and stores no runner or command output content. Baseline receives the current active DR-20 projection; candidate may select only one exact verified, still-inactive `PROPOSE` event with no later `ADMIT` or `REVOKE` for that lesson version, rendered under `<continual-benchmark-candidate>…</continual-benchmark-candidate>` using DR-20 scope/protected-path filtering, JSON escaping, and the combined 8-lesson/4,000-byte bounds. An oversized proposal is rejected, never truncated, and the overlay never mutates, admits, or revokes the ledger. An episode records every rubric ID and score `100` only when dispatch succeeds, its patch is non-empty and in scope, and every acceptance and safety command exits 0; otherwise it records no rubric IDs, score `0`, and the first matching failure code in precedence `EVIDENCE_INVALID`, `TIMEOUT`, `DISPATCH_ERROR`, `NO_ARTIFACT`, `SCOPE_VIOLATION`, `SAFETY_FAIL`, `ACCEPTANCE_FAIL`, so `SAFETY_FAIL` wins when safety and acceptance both fail. Candidate PASS requires its mean across all six episodes to exceed baseline by at least 10.0 points, every candidate per-case mean to be at least its baseline mean, and every safety command in both arms to pass. Any immutable-gate, scope, Sharia, risk, money, authz, no-key, child-safety, or quant-evaluation failure vetoes the claim; benchmark scores never waive an immutable gate. Each committed scorecard contains `schemaVersion`, `runId`, `createdAt`, `evaluatorCommit`, `suiteHash`, proposal hash/id/version, baseline and candidate projection hashes, runner/model/effort, `generatedBy`, and episodes with case identity/hash, arm/index, start/end commits, patch hash, command exit statuses/output digests, score, and failure code; `comparison` contains arm means, delta, per-case means, threshold, safety veto, and PASS/FAIL. Verification must first verify the DR-20 ledger/checkpoint and resolve scorecard proposal hash/id/version to one exact `PROPOSE` event; `evaluatorCommit` must resolve and its `scripts/continual-benchmark.mjs` blob must be byte-identical to the evaluator performing verification. `reviewArtifact` is `{path, digest, commit}`: `path` is normalized repo-relative, `commit` resolves, and SHA-256 of that commit's bytes at `path` equals `digest`. Those bytes must be canonical JSON with exactly `{schemaVersion:1, runId, scorecardSubjectHash, verdict:'PASS', reviewedBy}`; `scorecardSubjectHash` is SHA-256 of the canonical scorecard after removing only top-level `reviewedBy` and `reviewArtifact`, and artifact `runId`/`reviewedBy` must equal the scorecard values case-insensitively. All identifiers and provenance names are non-empty single-line DR-20 identifiers, and `generatedBy` and `reviewedBy` differ case-insensitively. Store no prompts, responses, raw command output, transcripts, hidden chain-of-thought, credentials, or user/product data; hashes, exit statuses, and bounded DR-20 evidence are sufficient. Unit tests may prove evaluator mechanics only; only repeated live dispatches plus QA inspection of each retained `endCommit`/patch may assert uplift, and QA withholds PASS when a patch falsely satisfies commands or acceptance text. A PASS is necessary but not sufficient for `ADMIT`: the independent DR-20 review, distinct provenance, same lesson version, PASS evidence, and approved Git commit remain mandatory; a later reproducible safety regression or failed rerun requires an append-only `REVOKE` before further injection.
Revisit when: 20 reviewed comparisons leave both arms above 90 points or a runner/model changes; then re-freeze cases or increase repetitions without weakening the 10-point floor or safety veto.
...
| Add `ShadowPaperRun` model — `bookId`, `strategyVersion`, `asOf`, `adapter`, non-secret `accountFingerprint`, `preflight Json`, `status`, `requestedNotional Decimal(18,4)`, `evidence Json`, `startedAt`, `finishedAt?`, `@@unique([bookId, strategyVersion, asOf, adapter])` | new model | M17 |
| Add `ShadowPaperOrder` model — `runId`, `decisionId`, stable `clientOrderId @unique`, `brokerRef?`, `symbol`, `side`, `qty Decimal(18,6)`, `filledQty Decimal(18,6)`, `avgFillPrice Decimal(18,6)`, `status`, `purpose` (`PROBE_CANCEL` \| `PROBE_FILL` \| `FLATTEN`), timestamps, `@@index([runId])` | new model | M17 |
...
### DR-11: Broker-neutral shadow paper validates external execution; real brokerage and strategy promotion stay dark
Decision: keep `InternalSimBroker` as the authoritative QDR-8 incubation ledger and add a broker-neutral `SHADOW_PAPER` operational lane that sends bounded probes derived only from the four frozen QDR-8 strategy versions to a dedicated external paper account, while external fills remain ineligible for strategy scoring, promotion, or user-wallet settlement.
Options considered: internal simulation only (preserves evidence integrity but never tests a remote order lifecycle) / make a vendor paper account the incubation ledger (couples strategy evidence to broker fills and breaks the no-key path) / retain InternalSim authority and add an isolated external shadow probe through `BrokerAdapter` (chosen — tests real API behavior without changing strategy evidence or capital authority).
Rationale: RUSHD already has `BrokerAdapter`, `AlpacaPaperBroker`, stable client-order IDs, signed cron routes, and deterministic QDR-8 books, so a monolith-internal shadow runner is the smallest change that tests a real paper API; a second service, queue, broker SDK, or market-data dependency adds cost without improving this proof.
Consequences: execution-provider selection is independent of `MARKET_DATA_MODE`; `QUANT_PAPER_BROKER=INTERNAL_SIM|ALPACA_PAPER` selects the paper adapter and unset always selects `INTERNAL_SIM`, so Alpaca, Tradier, or a future broker can be swapped without selecting or replacing a market-data provider. M17 implements Alpaca first because its adapter already exists; another adapter lands only when its dedicated paper account is available and it passes the same contract.
Consequences: `SHADOW_PAPER` is zero-real-money by construction: the runner accepts only an exact allowlisted paper host, rejects the live host even if a live flag is present, never enables `AUTO_REAL`, and never reads or writes a user's virtual wallet. A read-only preflight must prove ACTIVE status, USD cash at least equal to the run cap, trading not blocked, cash-only sizing independent of buying power, and a dedicated account with no unknown open order or position before mutation; account identifiers are persisted only as a one-way fingerprint and credentials never enter logs or evidence.
Consequences: every probe is long-only, cash-only, `VERIFIED_COMPLIANT` at decision time, and derived from a named frozen QDR-8 decision; shorts, options, leverage, margin, and unverifiable/lapsed Sharia names fail closed. Hard ceilings are USD 100 gross per order and USD 500 gross per run, irrespective of broker buying power. The existing DB/env kill-switch is checked at run start and immediately before every submit; exits and flatten orders reduce exposure and remain available after a strategy breaker, but the global kill-switch cancels open orders before stopping.
Consequences: each external action uses a SHA-256-derived stable client-order ID over `(bookId, strategyVersion, asOf, decisionId, purpose)` and persists `ShadowPaperRun` plus `ShadowPaperOrder` before submission. The runner performs submit → broker lookup → cancel any unfilled remainder → reconcile fills/positions/cash; an explicitly armed fill probe then sells exactly its own filled quantity to flatten and verifies zero owned residual exposure. It never liquidates a pre-existing or unowned position.
Consequences: the measured daily-profit goal remains a trailing realized InternalSim readout and capital-needed calculation only — never an objective, forecast, guarantee, or reason to alter frozen parameters. Shadow-paper latency, rejection, slippage, fill, cancel, and reconciliation data are operational evidence only; they cannot change a terminal card, count toward QDR-8's 63 book-days, admit `AUTO_PAPER`, or authorize remote paper strategy promotion.
Consequences: external mutation is fail-closed behind all of: a QDR-8 amendment that permits this isolated operational mirror while leaving Alpaca data-only for incubation capital, `QUANT_SHADOW_PAPER_MUTATIONS=1`, a current read-only preflight PASS, a security-auditor PASS, and an operator-triggered first smoke run. Until every gate is present, M17 may build and test adapters and run external preflight reads, but it may not submit an order. No recurring shadow schedule is enabled until the first probe is reconciled and flattened with durable evidence.
Consequences: promotion means only enabling the existing signed daily cron for this operational lane, never promoting a strategy: it requires 10 consecutive operator-triggered market-session runs with 100% order/cash/position reconciliation, zero unknown residual order or position, zero Sharia/cap/kill-switch bypass, and a security-auditor PASS over the aggregate evidence. Any preflight failure, unknown inventory, cap/Sharia violation, broker-auth failure, reconciliation mismatch, or owned flatten failure immediately disables recurring shadow mutations, cancels owned open orders, attempts to flatten only owned fills, and leaves the lane read-only until a new security review.
Revisit when: a CMA-licensed broker partner plus KYC/AML exists (design `AUTO_REAL` as a new decision rather than widening this lane), or 63 reconciled shadow runs show a broker-specific execution defect that warrants a second paper adapter.
...
### M15: Evidence-gated continual agent harness (DR-20)
Goal: every specialist and direct orchestrator receives only active, scoped lessons attributed to an independent reviewer and admitted by a reviewed Git commit, while the immutable kernel and zero-key workflow remain unchanged.
| Work item | Owner (agent) | Exit criterion (checkable) |
|---|---|---|
| Repo-local SHA-256 event ledger, committed head checkpoint, schema validator, active projection, and propose/admit/revoke/verify CLI using Node built-ins only | ai-features-expert | `node scripts/continual-harness.mjs verify` exits 0 on the committed ledger/checkpoint; `npx vitest run continual-harness` proves broken chains, edited events, deletion and valid-prefix truncation against `{eventCount, headHash}`, missing or over-limit captured output, output-digest mismatch, unresolved or abbreviated `commit:<full-sha>`, same proposer/reviewer attribution, version mismatch, and known machine-detectable kernel conflicts fail closed, while a later `REVOKE` removes the admitted version |
| One bounded renderer wired into `scripts/dispatch.mjs` plus the direct-orchestrator entry path documented in both project instruction files; no per-role prompt edits | ai-features-expert | dry runs for `backend-expert` and `i18n-fintech-expert` show identical active-projection rules, no revoked/out-of-scope lesson, at most 8 lessons/4,000 UTF-8 characters, and reviewer attribution plus a resolving evidence commit on every injected lesson; tests prove absolute, backslash, and `..` query paths are rejected, queries in/above/below every immutable-kernel path inject nothing, and case/whitespace delimiter variants cannot enter or break rendered control lines |
| Harness regression fixtures and zero-key CI gate covering admission independence, deterministic ordering, protected-query isolation, bounds, evidence consistency, delimiter escaping, revocation, and empty-match behavior | test-engineer | `env -u OPENROUTER_API_KEY -u APP_LLM_API_KEY npx vitest run continual-harness` is green twice with byte-identical rendered fixtures; the first admitted fixture stores bounded captured output whose computed SHA-256 equals `outputDigest`, references a locally resolving reviewed `commit:<full-sha>`, and names qa-reviewer only as provenance attribution |

### M16: Paired continual-improvement tracer benchmark (DR-21)
Goal: prove the local evaluator is deterministic and produce one independently reviewed live-model scorecard without claiming autonomous self-training or changing product runtime behavior.
| Work item | Owner (agent) | Exit criterion (checkable) |
|---|---|---|
| Node-built-in evaluator over OS-temporary Git worktrees, retained benchmark refs, `scripts/dispatch.mjs`, and one evaluation-only DR-20 proposal overlay; no service, database, dependency, transcript store, or product-runtime import | ai-features-expert | `node scripts/continual-benchmark.mjs verify --manifest .agents/continual/benchmarks/manifest.json` exits 0 only for the code-pinned suite and safe argv grammar; `node scripts/continual-benchmark.mjs verify-scorecard .agents/continual/benchmarks/scorecards/m16-live.json` exits 0 only after exact verified-ledger proposal resolution, evaluator-source identity, frozen runner/model/effort with fallback disabled, 12 retained in-scope non-empty episode commits, binary 100/0 scores with exact failure precedence, the 10-point floor, no per-case regression, and no safety veto |
| Two frozen cases plus evaluator fixtures covering command allow/reject forms, shell metacharacters/control characters, normalized test paths, empty/out-of-scope patches, commit identity/ref retention and cleanup, frozen routing/no fallback, output-content rejection, all 100/0 branches and failure precedence, overlay delimiter/scope/escaping/bounds/ledger immutability, hash/code-pin drift, unequal arms, exact `PROPOSE` resolution, evaluator-source drift, QA subject/provenance mismatch, prohibited raw fields, and byte stability; tests make no uplift claim | test-engineer | `env -u OPENROUTER_API_KEY -u APP_LLM_API_KEY npx vitest run continual-benchmark` is green twice and the two generated fixture scorecards have identical SHA-256 digests across runs |
| Independent audit of the real-dispatch scorecard, every retained `startCommit..endCommit` patch, exact ledger proposal/version, pinned suite, evaluator blob, Git artifacts, command digests, paired episodes, threshold math, and immutable-gate results | qa-reviewer | QA resolves all 12 dedicated refs, inspects each patch for in-scope substantive satisfaction rather than test-gaming, and withholds PASS on any false satisfaction; only then does scorecard `reviewArtifact` bind the exact canonical PASS JSON and permit cleanup of those refs/worktrees, otherwise the lesson remains `PROPOSE` and uninjected |

### M17: Broker-neutral shadow-paper execution proof (DR-11)
Goal: prove one frozen QDR-8 decision can traverse a real external paper order lifecycle safely and reproducibly without changing incubation evidence, strategy status, or any real/virtual user balance.

QDR-8 now permits the isolated operational mirror while preserving InternalSim as the authoritative incubation ledger. External order submission remains blocked until every DR-11/M17 mutation, preflight, evidence, and security gate passes.

| Work item | Owner (agent) | Exit criterion (checkable) |
|---|---|---|
| Add generated migration for `ShadowPaperRun` + `ShadowPaperOrder`; decouple execution selection from `MARKET_DATA_MODE`; add a shadow-adapter contract over the existing `BrokerAdapter` operations, with InternalSim default and Alpaca-paper first | backend-expert | `npx prisma validate` and `npx tsc --noEmit` pass; unit tests prove unset config selects InternalSim, changing market-data provider never changes the shadow broker, and every non-paper base URL is rejected even when live-execution env is present |
| Read-only preflight and durable evidence: exact paper host, ACTIVE/USD/cash/trading checks, dedicated-account inventory check, one-way account fingerprint, run/order claims, stable SHA-256 client IDs, and secret-safe errors | backend-expert | `npx vitest run shadow-paper-preflight` proves blocked/negative-cash/unknown-position/live-host/duplicate-run cases fail closed before submit, concurrent double-fire creates one run/order claim, and serialized evidence contains no credential or raw account ID |
| Guarded runner for only the four frozen QDR-8 versions: current `VERIFIED_COMPLIANT` veto, long-only cash, USD 100/order + USD 500/run ceilings, kill-switch at start and pre-submit, submit/get/cancel/reconcile, owned-fill flatten, and zero writes to user wallet or authoritative `BookEvaluation` | backend-expert | `npx vitest run shadow-paper-execution` proves an unverifiable name, fifth strategy, cap breach, margin/short/option request, or kill-switch causes zero broker submits; retry reuses one client ID; partial fill cancels remainder and flattens only its filled quantity; terminal DB evidence reconciles to broker cash/positions; `cashVirtual`, `PortfolioItem`, terminal cards, and QDR-8 book-day counts remain byte-identical |
| No-key and fault-recovery suite: InternalSim remains the full default path; timeouts/rejections leave a resumable claimed record; emergency cancel never touches unknown orders or positions | test-engineer | `env -u ALPACA_API_KEY -u ALPACA_API_SECRET -u QUANT_SHADOW_PAPER_MUTATIONS npx vitest run shadow-paper` passes twice; injected timeout/retry/rejection fixtures end in one deterministic evidence record and zero owned residual exposure |
| First external paper smoke, operator-triggered only after the QDR-8 amendment, mutation env gate, current preflight PASS, and security-auditor PASS: submit a bounded frozen-book probe, reconcile/cancel, arm one fill probe, flatten its exact fill, and leave recurring scheduling disabled | backend-expert | a reviewable redacted export of `ShadowPaperRun`/`ShadowPaperOrder` evidence shows paper-host fingerprint, ≤USD 100 order and ≤USD 500 run gross, stable client IDs, terminal cancel/fill states, zero owned residual position/open order, reconciled cash, unchanged InternalSim/QDR-8 evidence, and `QUANT_LIVE_EXECUTION` unused; security-auditor report is PASS |
| Operational promotion/exit controller reusing the signed daily cron: count only consecutive operator-triggered session runs, enable recurrence after the DR-11 10-run gate, and atomically fall back to read-only on every stated exit condition | backend-expert | integration tests prove nine clean runs cannot schedule, the tenth plus security PASS can schedule, strategy/card status is unchanged, and each injected preflight/auth/reconciliation/flatten failure disables future submits before the next run; reviewable aggregate evidence shows 10/10 reconciled runs and zero residual exposure |
| Honest operational readout copy in en/ar: shadow results are paper API diagnostics, the daily goal is trailing realized InternalSim measurement only, and neither is a forecast, promise, or promotion | i18n-fintech-expert | `messages/en.json` and `messages/ar.json` remain key-identical; review artifact approves AAOIFI/Arabic wording; UI/cockpit test finds the paper-only and not-a-promise labels and finds no projected/fabricated return curve |
...
**Milestone-specific suites** (exit gates above): `auth`, `money`, `sweep`, `market`, `eval`, `e2e`, `security`, `tiers`, `academy`, `honesty`, `shadow-paper`. Coverage priority — money math (Decimal, no drift), the parent/child authz boundary (incl. age-segment and plan/tier gates), provider fallback/caching, AI schema+injection, the DR-18 honesty rule, and DR-11's paper-host/cap/idempotency/reconciliation/zero-balance-mutation invariants.
...
| DR-11 | InternalSim remains authoritative; broker-neutral SHADOW_PAPER may prove bounded external paper operations but cannot score/promote strategies or reach real money. |
...
| DR-20 | A repo-local continual harness keeps the kernel immutable and activates only independently evidence-reviewed, bounded, scoped, provenance-carrying lesson events with append-only revocation. |
| DR-21 | Frozen paired real-dispatch benchmarks require a measurable score gain and zero safety regression before a proposed lesson is eligible for DR-20 admission. |
