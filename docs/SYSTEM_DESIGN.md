...
Consequences: each canonical-JSON case contains `caseId`, positive `caseVersion`, `role`, `task`, `primaryPath`, `taskTags`, `startCommit`, ordered `acceptanceCommands`, weighted binary `rubric`, and ordered `safetyCommands`; `caseHash` is SHA-256 of the DR-20 canonical serialization excluding `caseHash`, and `suiteHash` is SHA-256 of sorted `caseId@caseVersion:caseHash` lines. The computed `suiteHash` must equal a literal pinned in `scripts/continual-benchmark.mjs`; manifest and case hashes are integrity inputs, never authority, and changing a case, manifest, or pin requires an ordinary reviewed Git change. M16 freezes exactly two tracer cases; a proposal targeting an uncovered role cannot claim improvement or be admitted until a hash-pinned case for that role is reviewed and added. The evaluator creates clean temporary Git worktrees at each case's `startCommit` and runs three real episodes per case per arm through the same runner, model, effort, task, environment, and acceptance/safety commands; baseline receives the current active DR-20 projection, while candidate receives that projection plus exactly one referenced `PROPOSE` lesson through an evaluation-only `scripts/dispatch.mjs` path that never activates or writes the lesson. Episode score is `100 × passed rubric weight / total rubric weight`; candidate PASS requires its mean across all six episodes to exceed baseline by at least 10.0 points, every candidate per-case mean to be at least its baseline mean, and every safety command in both arms to pass. Any immutable-gate, scope, Sharia, risk, money, authz, no-key, child-safety, or quant-evaluation failure vetoes the claim; benchmark scores never waive an immutable gate. The closed failure taxonomy is `NONE`, `DISPATCH_ERROR`, `TIMEOUT`, `NO_ARTIFACT`, `SCOPE_VIOLATION`, `ACCEPTANCE_FAIL`, `SAFETY_FAIL`, and `EVIDENCE_INVALID`; dispatch/evidence failures invalidate the comparison rather than count as uplift. Each committed scorecard contains `schemaVersion`, `runId`, `createdAt`, `evaluatorCommit`, `suiteHash`, proposal hash/id/version, baseline and candidate projection hashes, runner/model/effort, `generatedBy`, and episodes with case identity/hash, arm/index, start/end commits, patch hash, command exit statuses/output digests, score, and failure code; `comparison` contains arm means, delta, per-case means, threshold, safety veto, and PASS/FAIL. Verification must first verify the DR-20 ledger/checkpoint and resolve scorecard proposal hash/id/version to one exact `PROPOSE` event; `evaluatorCommit` must resolve and its `scripts/continual-benchmark.mjs` blob must be byte-identical to the evaluator performing verification. `reviewArtifact` is `{path, digest, commit}`: `path` is normalized repo-relative, `commit` resolves, and SHA-256 of that commit's bytes at `path` equals `digest`. Those bytes must be canonical JSON with exactly `{schemaVersion:1, runId, scorecardSubjectHash, verdict:'PASS', reviewedBy}`; `scorecardSubjectHash` is SHA-256 of the canonical scorecard after removing only top-level `reviewedBy` and `reviewArtifact`, and artifact `runId`/`reviewedBy` must equal the scorecard values case-insensitively. All identifiers and provenance names are non-empty single-line DR-20 identifiers, and `generatedBy` and `reviewedBy` differ case-insensitively. Store no prompts, responses, raw command output, transcripts, hidden chain-of-thought, credentials, or user/product data; hashes and bounded DR-20 evidence are sufficient. Unit tests may prove hashing, pairing, scoring, taxonomy, and veto behavior only; only the repeated live-dispatch scorecard may assert model uplift. A PASS is necessary but not sufficient for `ADMIT`: the independent DR-20 review, distinct provenance, same lesson version, PASS evidence, and approved Git commit remain mandatory; a later reproducible safety regression or failed rerun requires an append-only `REVOKE` before further injection.
...
| Work item | Owner (agent) | Exit criterion (checkable) |
|---|---|---|
| Node-built-in evaluator over Git worktrees, `scripts/dispatch.mjs`, and one evaluation-only DR-20 proposal overlay; no service, database, dependency, transcript store, or product-runtime import | ai-features-expert | `node scripts/continual-benchmark.mjs verify --manifest .agents/continual/benchmarks/manifest.json` exits 0 only when the computed suite hash equals the evaluator-code pin and prints exactly two case hashes plus that pin; `node scripts/continual-benchmark.mjs verify-scorecard .agents/continual/benchmarks/scorecards/m16-live.json` exits 0 only after verified-ledger proposal resolution, evaluator-source identity, 3 baseline + 3 candidate episodes per case, the 10-point floor, no per-case regression, and no safety veto |
| Two frozen canonical cases plus evaluator fixtures covering hash drift, code-pin mismatch, unequal arms, projection mismatch, score arithmetic, every failure code, safety veto, exact `PROPOSE` resolution, evaluator-source drift, QA path/digest/commit or exact-field mismatch, unrelated valid QA blobs, scorecard-subject mismatch, case-insensitive identity mismatch/self-review, multiline/invalid identifiers, prohibited raw fields, and byte-stable scorecards; tests make no model-uplift claim | test-engineer | `env -u OPENROUTER_API_KEY -u APP_LLM_API_KEY npx vitest run continual-benchmark` is green twice and the two generated fixture scorecards have identical SHA-256 digests across runs |
| Independent audit of the real-dispatch scorecard, exact ledger proposal/version, pinned suite, evaluator blob, Git artifacts, command digests, six paired episodes per arm, threshold math, and immutable-gate results | qa-reviewer | Scorecard `reviewArtifact` binds a committed repo-relative canonical-JSON artifact with exactly `{schemaVersion:1, runId, scorecardSubjectHash, verdict:'PASS', reviewedBy}`; its run/reviewer identities match the scorecard case-insensitively under DR-20 identifier rules, its subject hash binds that scorecard excluding only `reviewedBy`/`reviewArtifact`, and the artifact commit/digest binds those exact bytes; otherwise the lesson remains `PROPOSE` and uninjected |
