...
import { ArrowRight, CheckCircle2, Compass, Crown, Target, XCircle, FlaskConical, BookOpen, Sparkles } from 'lucide-react';
...
export default function QuizListPage({
  ...
  searchParams?: { setupId?: string | string[]; topic?: string | string[]; tab?: string | string[] };
}) {
  ...
  const requestedTopic = typeof searchParams?.topic === 'string'
    ...
    : null;
  const defaultTab = requestedTopic ? 'quizzes' : requestedSetupId ? 'strategy' : 'strategy';

  const [activeTab, setActiveTab] = useState<'strategy' | 'quizzes' | 'committee'>(defaultTab);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-28 pt-8 text-foreground sm:px-6 sm:pt-12 md:pb-12 space-y-8">
      ...
      {/* Unified Learning Sub-Navigation Tabs */}
      <div className="flex items-center gap-2 p-1.5 bg-foreground/[0.03] dark:bg-white/[0.04] rounded-2xl border border-[var(--border-color)] overflow-x-auto">
        <button
          type="button"
        ...
        >
          ...
          <span>{locale === 'ar' ? 'معمل الاستراتيجيات' : 'Strategy Practice Lab'}</span>
        </button>
        ...
        >
          ...
          <span>{locale === 'ar' ? 'اختبارات ومسارات المعرفة' : 'Knowledge Quizzes & Paths'}</span>
        </button>
      ...
      {activeTab === 'quizzes' && (
        <div className="space-y-8">
          {lastResult ? (
            <section
              className={`flex flex-col gap-4 rounded-2xl px-5 py-4 sm:flex-row sm:items-center sm:justify-between ${lastResult.passed ? 'bg-up/10' : 'bg-down/10'}`}
              aria-live="polite"
            >
          ...
          <div className="space-y-14" aria-label={t('pathLabel')}>
