    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/[0.02] rounded-2xl p-3 border border-white/[0.04] flex items-center justify-between text-xs text-gray-400 shadow-sm">
        <div className="flex items-center space-x-2 rtl:space-x-reverse">
          ...
          <span>
            {marketTab === 'TASI'
              ? (isAr ? 'السوق مغلق. يبدأ التداول الأحد' : 'Market closed. Trading starts Sunday')
              : (isAr ? 'السوق مغلق. يبدأ التداول الإثنين' : 'Market closed. Trading starts Monday')}
          </span>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-3 overflow-x-auto pb-2 pr-1 no-scrollbar rtl:space-x-reverse">
          {activeTickers.filter(t => t.pct > 0).map((t) => (
            <button
              key={t.symbol}
              onClick={() => handleSelectSymbol(t.symbol, marketTab)}
            ...
            >
              ...
              <p className="text-xs font-bold text-gray-300 truncate w-full text-center">{isAr ? t.arName : t.name}</p>
              ...
              <p className="text-[10px] font-semibold text-emerald-400 text-center">+{t.pct.toFixed(2)}%</p>
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="glass-panel p-6 rounded-3xl border border-white/5 bg-black/40 min-h-[600px]">
          {loadingStock ? (
            ...
            <div className="flex items-center justify-center h-[500px] text-gray-500">
              {isAr ? 'اختر شركة لبدء التحليل والتداول' : 'Select a stock to view details'}
            </div>
          )}
        </div>
      </div>
